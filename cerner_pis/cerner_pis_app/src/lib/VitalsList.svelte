<script lang="ts">
  ...
  import {
    Scale, Ruler, Calculator, Activity, Gauge,
    Thermometer, Droplets, Wind, ClipboardList, X,
  } from 'lucide-svelte';
  ...
  const DEFAULT_CONFIG = {
    label: 'Vital',
    Icon: ClipboardList as Component,
  ...
  function unitStr(obs: Observation): string {
    ...
    if (obs.component?.length)   return obs.component[0]?.valueQuantity?.unit ?? '';
    return '';
  }
  ...
  function latestPerType(list: Observation[]): Observation[] {
    ...
    return list.filter(hasValue).filter(obs => {
  ...
  function closeHistory() { selectedType = null; }
</script>

<!-- ─── Current Vitals Grid ───────────────────────────────────────────────── -->
<section>
  <h2 class="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-4">Current Vitals</h2>
  ...
  {:else}
    <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
      {#each latestPerType($vitals) as obs (obs.id)}
        ...
        <button
          class="bg-white rounded-xl border-l-4 {cfg.color} border border-slate-100 shadow-sm hover:shadow-md transition-all text-left p-4 flex flex-col gap-3"
          onclick={() => openHistory(vitalName(obs))}
        >
          <!-- Icon + label side by side -->
          <div class="flex items-center gap-2">
            <div class="w-7 h-7 rounded-md bg-slate-50 flex items-center justify-center shrink-0 {cfg.textColor}">
              <svelte:component this={cfg.Icon} size={14} strokeWidth={1.5} />
            </div>
            <span class="text-[11px] font-semibold uppercase tracking-widest text-slate-500 leading-tight">
              {cfg.label}
            </span>
          </div>
          ...
          <!-- Value -->
          <div>
            <span class="text-2xl font-bold {cfg.textColor} leading-none tracking-tight">
              {valueStr(obs)}
            </span>
            {#if unitStr(obs)}
              <span class="text-xs font-medium text-slate-400 ml-1">{unitStr(obs)}</span>
            {/if}
          </div>

          <!-- Date + reading count -->
          <div class="flex items-center justify-between pt-2 border-t border-slate-100">
            <p class="text-[11px] text-slate-400">{formatDate(obs.effectiveDateTime)}</p>
            {#if historyFor($vitals, vitalName(obs)).length > 1}
              <span class="text-[10px] text-slate-400 font-medium">
                {historyFor($vitals, vitalName(obs)).length} readings
              </span>
            {/if}
          </div>
        </button>
...
{#if $vitals.filter(hasValue).length > 0}
  <section class="mt-8">
    <h2 class="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-4">Recent Entries</h2>
            ...
            >
              <td class="px-5 py-3">
                <div class="flex items-center gap-2.5">
                  <span class="{cfg.textColor}">
                    <svelte:component this={cfg.Icon} size={15} strokeWidth={1.5} />
                  </span>
              ...
              <td class="px-5 py-3 hidden md:table-cell">
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium
                  {obs.status === 'final' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}">
                  {obs.status ?? '—'}
                </span>
              </td>
            </tr>
...
{#if selectedType}
  ...
  <div class="fixed inset-0 bg-black/20 z-40" onclick={closeHistory}></div>
  ...
  <div class="fixed right-0 top-0 h-full w-full max-w-sm bg-white z-50 shadow-2xl flex flex-col">
    ...
    <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50">
      <div class="flex items-center gap-3">
        <div class="w-9 h-9 rounded-lg bg-white border border-slate-200 flex items-center justify-center {panelCfg.textColor}">
          <svelte:component this={panelCfg.Icon} size={18} strokeWidth={1.5} />
        </div>
        <div>
          <p class="font-semibold text-slate-800 text-sm">{panelCfg.label}</p>
          <p class="text-xs text-slate-400">{historyFor($vitals, selectedType).length} readings</p>
        </div>
      </div>
      <button
        class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:bg-slate-200 transition-colors"
        ...
        {#each historyFor($vitals, selectedType) as obs (obs.id)}
          ...
          <li class="px-6 py-4">
            <div class="flex items-start justify-between gap-3">
              <div>
                <p class="text-xl font-bold {cfg.textColor} tracking-tight">
                  {valueStr(obs)}
